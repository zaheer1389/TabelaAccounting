...
public class DateRangeLayout extends VBox {
	public Button btnReport;
	public DatePicker fromDate;
	public DatePicker toDate;

	public DateRangeLayout() {
		...
		EventHandler<KeyEvent> handler = new EventHandler<KeyEvent>() {
			public void handle(KeyEvent arg0) {
				if (arg0.getCode() == KeyCode.ENTER) {
					if (DateRangeLayout.this.fromDate.getValue() == null) {
						FXOptionPane.showErrorDialog(null, "Please enter from date", "Error");
						DateRangeLayout.this.fromDate.requestFocus();
					} else if (DateRangeLayout.this.fromDate.getValue() == null) {
						FXOptionPane.showErrorDialog(null, "Please enter to date", "Error");
						DateRangeLayout.this.toDate.requestFocus();
					} else {
						DateRangeLayout.this.btnReport.fire();
					}
				}
			}
		};
